...
if (gadgetHandler:IsSyncedCode()) then
	...
	if not chickenTeamID then
		for humanTeam1,_ in pairs(humanTeams) do
			 for humanTeam2,_ in pairs(humanTeams) do
				 if not Spring.AreTeamsAllied(humanTeam1,humanTeam2) and Spring.GetModOptions()["materialstorage"] then
					 return false-- no chickens because we found a pair of players (or non-chicken bots) that are not allied
				 end
			 end
		end
	...
	SetGlobals(luaAI or "Survival Spawner: Normal") -- set difficulty

		
		-- Spring.Echo("------chickenSwitches-------")		
		-- for i, v in pairs(chickenSwitches) do
			-- Spring.Echo("---------------")
			-- Spring.Echo(modes[i])
			-- for k, vv in pairs(chickenSwitches[i]) do
	...
	local function ChooseTarget()
		...
		--Spring.Echo(targetCache, " targetcache Value")
		
		if (targetCacheCount >= nextSquadSize) or (targetCache and GetUnitIsDead(targetCache)) then
			local tries = 0
				...
				repeat			
					...
					tries = (tries + 1)
				until ((targetCache and not GetUnitIsDead(targetCache)) and 
				(targetCache and not GetUnitNeutral(targetCache)) and 
				slowunit) or (tries > maxTries)
	...
	local function SpawnTurret(burrowID, turret)
		...
		until (Spring.TestBuildOrder(UnitDefNames[turret].id, x, 0, z, 0) == 2 or tries > maxTries)
		--Spring.Echo(turret)	
		y = GetGroundHeight(x,z)
	...
	local function SpawnBurrow(number)
		...
		elseif ((burrowSpawnType == "alwaysbox") and (tries < (maxTries / 2))) then
			...
			x = mRandom(spawnSquare*2, MAPSIZEX - spawnSquare*2)
			z = mRandom(spawnSquare*2, MAPSIZEZ - spawnSquare*2)
		end
		
		local y = GetGroundHeight(x, z)
		tries = tries + 1
		local blocking = TestBuildOrder(unitDefID, x, y, z, 1)
	...
	function gadget:GameStart()
		...
		if warningMessage then 
			...
			Spring.Echo("(Assigning Chicken Team to Gaia)")
	...
	function gadget:GameFrame(n)
		...
		if (chickenCount < maxChicken) then
			...
			if i and defs then
				...
				if x and y and z then
					...
					--if (chickenSwitches[difficulty][defs.unitName]) then
					--Spring.Echo("difficulty", chickenSwitches[difficulty], difficulty)
					if (chickenSwitches[difficulty] and chickenSwitches[difficulty][defs.unitName])then
						--Spring.Echo("Match found", defs.unitName, " at difficulty", difficulty)
						myUnit = chickenSwitches[difficulty][defs.unitName]
						--Spring.Echo("Spawning alternate unit", myUnit)
					end
					--end
		...
		-- Spring.Echo("endscores")
		-- for k,v in pairs(endScores) do
			-- Spring.Echo(k,v)
		-- end	
		...
		SendToUnsynced("PassQueenTime",math.floor(queenTime - queenAnger - t), math.floor(queenTime))
	end
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
	function POSToLuaUI(_, timeRemaining, timeTillQueen)
		--Spring.Echo("looking for luaUI w round",timeRemaining, timeTillQueen)
		if (Script.LuaUI('PassQueenTime')) then
		--Spring.Echo("Found luaUI")
			--if(team == spGetLocalTeamID() ) then
				Script.LuaUI.PassQueenTime(timeRemaining, timeTillQueen)
			--end	
		end
	end
	...
	function gadget:Initialize()
		...
		gadgetHandler:AddSyncAction('PassQueenTime',POSToLuaUI)			
	end
