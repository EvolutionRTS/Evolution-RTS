...
--Game Config ------------------------------------
local unitList = {}
unitList["EVORTS"] = {} --initialize table
unitList["EVORTS"]["esilo"] = {}
...
function drawButton( )
 ...
 -- draw icon
   ...
   gl.Color(1,0,0, alpha or 1)
   gl.Text( readyNukeCount, centerx, ymin + intConfig["buttonCurrentHeight"], intConfig["fontSize"], "nc")
   gl.Color(1,1,1,1)

   if ( highProgress >= 0 ) then
      gl.Color(1 - highProgress, 1 * highProgress,0, alpha or 1)
      gl.Text( string.format( "%.0f", highProgress * 100 ) .. "%", centerx + 0.1 * intConfig["buttonCurrentWidth"], ymin + 0.1 * intConfig["buttonCurrentHeight"], 0.9 * intConfig["fontSize"], "nc")
      gl.Color(1,1,1,1)
   end
end
