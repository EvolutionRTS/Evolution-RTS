...
local changelogFile = VFS.LoadFile("howtoplay.txt")

local bgMargin = 6

local closeButtonSize = 30
local screenHeight = 520-bgMargin-bgMargin
local screenWidth = 600-bgMargin-bgMargin
...
function widget:DrawScreen()
    ...
    if show or showOnceMore then
		...
		if (WG['guishader_api'] ~= nil) then
			...
			WG['guishader_api'].InsertRect(rectX1, rectY2, rectX2, rectY1, 'howtoplaylist')
		end
		showOnceMore = false
		
    else
		if (WG['guishader_api'] ~= nil) then
			WG['guishader_api'].RemoveRect('howtoplaylist')
		end
	end
end
...
function widget:Initialize()
	if changelogFile then
		...
		Spring.Echo("Commands info: couldn't load the howtoplay file")
