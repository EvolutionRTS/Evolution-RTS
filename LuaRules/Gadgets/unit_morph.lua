...
function gadget:GameFrame(n)
  if ((n+24)%150<1) then
	...
	for unitID in pairs(tobemorphed) do
		local x, y, z =  GetUnitPosition(unitID)
		--if (y ~= nil) and (y <= -30) then
		if (y <= -30) then
			StartMorph(unitID, tobemorphed[unitID][1], tobemorphed[unitID][2], tobemorphed[unitID][3])
			tobemorphed[unitID] = nil
		end
	end
...
function gadget:CommandFallback(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions)
  ...
  if (not morphData) then
		if UnitDefs[unitDefID]["name"] ~= 'esubplane' then
			StartMorph(unitID, unitDefID, teamID, morphDef)
		else
			...
			--	tobemorphed = 
				...
				tobemorphed[unitID] = {unitDefID, teamID, morphDef}
			end
		end
    return true, true
  end
