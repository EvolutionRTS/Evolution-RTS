...
function TriggerHandler:Update()
	...
	-- if frame%600 == (self.ai.id*15) then
		-- local x,y,z
		-- local comms = Spring.GetTeamUnitsByDefs(self.ai.id, {UnitDefNames.ecommanderbattleai.id, UnitDefNames.ecommanderhealerai.id, UnitDefNames.ecommanderbuildai.id, UnitDefNames.ecommandershieldai.id, UnitDefNames.ecommandercloakai.id, UnitDefNames.ecommanderfactoryai.id })
		-- if comms[1] then
