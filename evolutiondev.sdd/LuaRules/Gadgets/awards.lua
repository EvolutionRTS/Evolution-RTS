...
function gadget:Initialize()
	...
	for i=1,#WeaponDefs do
		if (WeaponDefs[i].type=="Flame" or 
			...
			WeaponDefs[i].name:lower():find("flame")) then --// == flamethrower or napalm
...
function gadget:GameFrame(n)
	...
	if not sentAwards then 
		...
		if pwnTeam then
			awardAward(pwnTeam, 'pwn', 'Damage dealt: '.. comma_value(maxDamage))
		end
		--[[
		...
		--]]
		if airTeam and maxAirDamage > getMeanDamageExcept(airTeam) * ef_10 then
		...
		--]]
		if nuxTeam and maxNuxDamage > getMeanDamageExcept(nuxTeam) * ef_10 then
			awardAward(nuxTeam, 'nux', 'Damage: '.. comma_value(maxNuxDamage))
		end
		if shellTeam and maxStaticDamage > getMeanDamageExcept(shellTeam) * ef_10 then
			awardAward(shellTeam, 'shell', 'Damage: '.. comma_value(maxStaticDamage))
		end
		if kamTeam and maxKamDamage > getMeanDamageExcept(kamTeam) * ef_5 then
			awardAward(kamTeam, 'kam', 'Damage: '.. comma_value(maxKamDamage))
		end
		if fireTeam and maxFireDamage > getMeanDamageExcept(fireTeam) * ef_5 then
			awardAward(fireTeam, 'fire', 'Damage: '.. comma_value(maxFireDamage))
		end
		if empTeam and maxEmpDamage/10 > getMeanDamageExcept(empTeam) * ef_5 then
		...
		if ouchTeam then
			awardAward(ouchTeam, 'ouch', 'Damage taken: '.. comma_value(maxOuchDamage))
		end
